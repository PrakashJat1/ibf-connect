...
const ManagementBatchPage = () => {
  ...
  function getAssignedTrainers() {
    const set = new Set();

    batches?.forEach((batch) => {
      if (batch?.technicalTrainer?._id) set.add(batch?.technicalTrainer?._id);
      if (batch?.softskillTrainer?._id) set.add(batch?.softskillTrainer?._id);
      if (batch?.aptitudeTrainer?._id) set.add(batch?.aptitudeTrainer?._id);
    });
...
export default ManagementBatchPage;
