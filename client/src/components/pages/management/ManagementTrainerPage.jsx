...
const ManagementTrainerPage = () => {
  ...
  const checkAssignedBatch = (trainer) => {
    if (trainer.assigned_Batches.length === 0)
      return toast.warn("No Batches for remove");
    else return setRemoveAssignedBatchesmodal(true);
  };
  useEffect(() => {
    fetchAllTrainers();
  }, []);
        ...
        <div className="d-flex justify-content-center">
          <button
            ...
            onClick={() => {
              ...
              checkAssignedBatch(row.original);
            }}
          >
            <UserMinus size={16} />
          </button>
...
export default ManagementTrainerPage;
