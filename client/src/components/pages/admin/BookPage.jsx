...
const BookPage = () => {
  ...
  const [deleteBookIssueModal, setDeleteBookIssueModal] = useState(false);
  ...
  const issuedBookColumns = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.issuedOn ? dateFormatter(row.original.issuedOn) : "—",
    },
    {
      ...
      cell: ({ row }) =>
        row.original.returnedOn ? dateFormatter(row.original.returnOn) : "—",
    },
    {
      accessorKey: "actions",
      header: () => (
  ...
  const deleteAllBooksIssues = async (rows) => {
    setSelectedRow([]);
    ...
    try {
      await bookissueService.deleteAll(data);

      toast.success("Books issued data deleted successfully");
    } catch (error) {
      console.log("Erorr in deleteAllBooks", error);
      toast.error("Error in deleteAllBooks");
    } finally {
      fetchAllIssuedBooks();
      setDeleteBookIssueModal(false);
    }
  };

  const deleteAllBooks = async (rows) => {
    setSelectedRow([]);
    const ids = rows.map((row) => row._id);
    const data = {
      ids,
    ...
    try {
      await bookService.deleteAll(data);
  ...
  return (
    <>
      ...
      <DataTable
        ...
        deleteButton="true"
        onDelete={(rows) => {
          setSelectedRow(rows), setDeleteBookIssueModal(true);
        }}
      ...
      <DataTable
        ...
        onDelete={(rows) => {
          setSelectedRow(rows), setDeleteBookModal(true);
        }}
      />
      ...
      {/* Confirm Book Modal */}
      <ConfirmModal
        show={deleteBookModal}
        title={"Confirm Deletion"}
        message={"Do you want to permanently delete selected Books"}
        onConfirm={() => {
          deleteAllBooks(selectedRow),
            setDeleteBookModal(false),
            setSelectedRow(null);
        }}
        onClose={() => {
          setDeleteBookModal(false), setSelectedRow(null);
        }}
      />

      {/* Confirm Book Issue Modal */}
      <ConfirmModal
        show={deleteBookIssueModal}
        title={"Confirm Deletion"}
        message={"Do you want to permanently delete selected Books Issue Data"}
        onConfirm={() => {
          deleteAllBooksIssues(selectedRow),
            setDeleteBookIssueModal(false),
            setSelectedRow(null);
        }}
        onClose={() => {
          setDeleteBookIssueModal(false), setSelectedRow(null);
        }}
      />
