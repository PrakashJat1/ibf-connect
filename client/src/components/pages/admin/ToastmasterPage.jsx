...
const ToastmasterPage = () => {
  ...
  const deleteToastmasterSessions = async (rows) => {
    setSelectedRow([]);
    ...
    try {
      await toastmasterService.deleteToastmasterSessions(data);
      toast.success("Sessions deleted successfully");
      fetchAllToastMasters();
    } catch (error) {
      console.log("Erorr in deleteToastmasterSessions", error);
      toast.error("Error in deleteToastmasterSessions");
    }
  };
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message={"Do you want to permanently delete selected Assessments"}
        onConfirm={() => {
          deleteToastmasterSessions(selectedRow),
